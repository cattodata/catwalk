...
/**
 * POST /api/resend
 * Real bulk email via Resend (resend.com, free tier 100/day, 3000/month).
 * Falls back to a stub response when RESEND_API_KEY is missing — UI flow stays
 * intact for demo without the env var.
 */
...
function applyMerge(html: string, recipient: Recipient): string {
  return html
...
async function sendOne(
  ...
  html: string,
  recipient: Recipient,
): Promise<{ accepted: boolean; id?: string; error?: string }> {
  try {
    ...
    const data = (await res.json().catch(() => ({}))) as ResendOk
    if (!res.ok) {
